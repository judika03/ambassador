...
func (cept *Intercept) removeMapping() error {
	var err error
	err = nil

	if cept.mappingExists {
		cept.p.Logf("%v: Deleting mapping", cept.ii.Name)
		delete := cept.cluster.GetKubectlCmd(cept.p, "delete", "mapping", fmt.Sprintf("%s-mapping", cept.ii.Name))
		delete.Stdout = os.Stdout
		delete.Stderr = os.Stderr
		err = delete.Run()
		cept.p.Logf("%v: Deleted mapping", cept.ii.Name)
	}
...
func MakeIntercept(p *supervisor.Process, out *Emitter, tm *TrafficManager, cluster *KCluster, ii *InterceptInfo) (*Intercept, error) {
	...
	cept := &Intercept{p: p, ii: ii, tm: tm, cluster: cluster, port: port}
